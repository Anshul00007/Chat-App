...
wss.on("connection", (socket) => {
    ...
    try {
      ...
      if (parsedMessage.type === "create") {
        ...
        if (rooms.has(roomName)) {
          ...
          socket.send(JSON.stringify({ type: "success", message: `Room "${roomName}" created.` }));
        ...
        if (rooms.has(roomId)) {
          ...
          socket.send(JSON.stringify({ type: "joined", message: `Joined room ${roomId}` }));
        } else {
        ...
        if (currentUserRoom) {
          for (let i = 0; i < everySocket.length; i++) {
            if (everySocket[i].room === currentUserRoom) {
  ...
  socket.on("close", () => {
    const index = everySocket.findIndex((user:any) => user.socket === socket);
    if (index !== -1) {
